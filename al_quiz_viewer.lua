...
local AL_QuizViewer = InputContainer:extend{
...
function AL_QuizViewer:init()
    ...
    self.height = self.height or math.floor(Screen:getHeight() * 0.8)
...
function AL_QuizViewer:_buildUI()
    ...
    local prefix = self.opts.title or "Quiz"
    local title_text = prefix .. " — " .. self.current_index .. "/" .. total
...
function AL_QuizViewer:_buildQuestionHTML(q, idx)
    local parts = {}

    local type_labels = {
        multiple_choice = "Múltipla Escolha",
        true_false = "Verdadeiro/Falso",
        short_answer = "Discursiva",
        essay = "Reflexão",
    }
    table.insert(parts, "<h3>" .. (type_labels[q.type] or q.type) .. "</h3>")

    -- Simple HTML for question text (without MD dependency)
    table.insert(parts, "<p>" .. q.question:gsub("\n", "<br>") .. "</p>")

    if (q.type == "multiple_choice" or q.type == "true_false") and q.options then
    ...
    if self.revealed[idx] then
        ...
        if q.type == "multiple_choice" or q.type == "true_false" then
            table.insert(parts, "<p><b>Correta: " .. (q.correct or "?") .. "</b></p>")
        ...
        elseif q.type == "essay" then
            if q.key_points and #q.key_points > 0 then
                table.insert(parts, "<p><b>Reflexão Recomendada:</b></p><ul>")
                for _ki, kp in ipairs(q.key_points) do
                    table.insert(parts, "<li>" .. kp .. "</li>")
                end
                table.insert(parts, "</ul>")
...
function AL_QuizViewer:_buildQuestionButtons(q, idx)
    local buttons = {}
    local self_ref = self
...
function AL_QuizViewer:_selectMCAnswer(idx, letter)
...
function AL_QuizViewer:_revealAnswer(idx)
    self.revealed[idx] = true
    self:_saveState()
    self:_refresh()
end

function AL_QuizViewer:_selfGrade(idx, is_correct)
    self.correct[idx] = is_correct
    self:_saveState()
    self:_refresh()
end

function AL_QuizViewer:_saveState()
...
function AL_QuizViewer:_showQuestionPicker()
    local questions = self.quiz_data.questions or {}
    local buttons = {}
    local self_ref = self
...
function AL_QuizViewer:_showCompletion()
    self.phase = "complete"
    self:_saveState()
    self:_refresh()
end

function AL_QuizViewer:_buildCompletionUI()
    local questions = self.quiz_data.questions or {}
    local total = #questions
    ...
    local button_rows = {
        {
            {
                text = _("Copy"),
                callback = function() self_ref:_copyResults() end,
            },
            {
                text = _("Export"),
                callback = function() self_ref:_exportResults() end,
            },
        },
        {
            {
            ...
            {
                text = _("Close"),
                callback = function() self_ref:onClose() end,
            },
        },
    }
    ...
    local text_frame = FrameContainer:new{
        padding = self.text_padding, margin = self.text_margin, bordersize = 0, html_widget,
    }

    self.frame = FrameContainer:new{
        radius = Size.radius.window, padding = 0, margin = 0, background = Blitbuffer.COLOR_WHITE,
        VerticalGroup:new{
            titlebar,
            CenterContainer:new{ dimen = Geom:new{ w = self.width, h = text_frame:getSize().h }, text_frame },
            CenterContainer:new{ dimen = Geom:new{ w = self.width, h = button_table:getSize().h }, button_table },
        },
    }

    self.movable = MovableContainer:new{ ignore_events = { "swipe", "hold", "touch", "pan" }, self.frame }
    self[1] = WidgetContainer:new{ align = self.align, dimen = self.region, self.movable }
end

function AL_QuizViewer:_refresh()
    self:_buildUI()
    UIManager:setDirty(self, "partial")
end

function AL_QuizViewer:_buildResultText()
    local questions = self.quiz_data.questions or {}
    local lines = {}
...
function AL_QuizViewer:_copyResults()
    Device.input.setClipboardText(self:_buildResultText())
    UIManager:show(InfoMessage:new{ text = _("Quiz results copied to clipboard."), timeout = 2 })
end

function AL_QuizViewer:_exportResults()
    local text = self:_buildResultText()
    local filename = "Quiz_Export_" .. os.date("%Y%m%d_%H%M%S") .. ".md"
    local export_dir = require("datastorage"):getDataDir() .. "/al_exports"
    require("libs/libkoreader-lfs").mkdir(export_dir)
...
function AL_QuizViewer:_confirmClose()
    self:onClose()
end

function AL_QuizViewer:onClose()
...
function AL_QuizViewer:onCloseWidget()
    UIManager:setDirty(nil, function() return "partial", self.frame and self.frame.dimen end)
end

function AL_QuizViewer:onShow()
    UIManager:setDirty(self, function() return "partial", self.frame and self.frame.dimen end)
    return true
end

function AL_QuizViewer:onTapClose(arg, ges_ev)
    if self.frame and ges_ev.pos:notIntersectWith(self.frame.dimen) then self:_confirmClose() end
    return true
end

function AL_QuizViewer:onSwipe(arg, ges)
    if self.frame and ges.pos:intersectWith(self.frame.dimen) then
        local direction = BD.flipDirectionIfMirroredUILayout(ges.direction)
        if direction == "west" then self:_navigate(1)
        elseif direction == "east" then self:_navigate(-1) end
    end
    return true
end

return AL_QuizViewer
